...
public class LavaFlyEnemy : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        rb.gravityScale = 0f;
        rb.collisionDetectionMode = CollisionDetectionMode2D.Continuous; // برای دقت بیشتر برخورد
    }
    ...
    private void FaceTarget()
    {
        ...
        if (targetPlayer.position.x > transform.position.x)
            scale.x = Mathf.Abs(scale.x); // رو به راست
        else
            scale.x = -Mathf.Abs(scale.x); // رو به چپ
